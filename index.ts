...
function _initMain() {
    ...
    let htmlBtn = new wa.ElementManager('#htmlInput').event('click', async (self) => {
        ...
        if(!file || !wa.Utils.checkFileExtension(file.name, ['html'])) return $.utils.WarningMsg('must be html file');
    ...
    let cssBtn = new wa.ElementManager('#cssInput').event('click', async (self) => {
            let file = await $.utils.GetFileFromUser();
            if(!file || !wa.Utils.checkFileExtension(file.name, ['css'])) return $.utils.WarningMsg('must be css file');
    ...
    let jsBtn = new wa.ElementManager('#jsInput').event('click', async (self) => {
        let file = await $.utils.GetFileFromUser();
        if(!file || !wa.Utils.checkFileExtension(file.name, ['js'])) return $.utils.WarningMsg('must be js file');
    ...
    function triggerEvent() {
        ...
        switch(dev_selector._val()) {
            case 'builder':
                ...
                if(val == 'theme') organizer.Theme?.showBuilder();
                else organizer.Theme?.actions.find(act => act.name == val)?.showBuilder();
                break;
            case 'themeElement':
    ...
    async function loadJS(text:string) {
        organizer.reset();
        script.remove();
    ...
    async function JSreplacer(text:string):Promise<string> {
        const mods:Array<{from:string,to:string}> = [
            {from: '../../WebApp.class.js', to: './dist/WebApp.class.js'},
            {from: '../../devtool.js', to: './dist/devtool.js'},
            {from: '../../index.js', to: './dist/index.js'},
        ]; 
        for await (const mod of mods) text = text.replace(mod.from, mod.to);
        text = `${text} \n\n `;
        return text;
    }

    async function HTMLReplacer(text:string) {
        const mods:Array<{from:string,to:string}> = [
            { from: '__WC_ID__', to: 'htmlSection_' }
        ];
        for await (let mod of mods) text = text.replaceAll(mod.from, mod.to);
        return text;
    }
