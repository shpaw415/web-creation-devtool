...
class Utils
{
    ...
    public function Send_content($file) {
        if (file_exists($file)) {
    ...
    public function file_type($file) {
        ...
        $video_ext = ['webm', 'mp4'];
        if (in_array($ext, $this->img_ext)) return 'image';
        elseif (in_array($ext, $this->video_ext)) return 'video';
        elseif (in_array($ext, $this->audio_ext)) return 'audio';
        elseif (in_array($ext, $this->text_ext)) return 'text';
        else return 'file';
    }
    public static function ReceiveFile($file, string $path, string $type) {
        if ($type == 'image') {
            $ext = pathinfo($file['name'], PATHINFO_EXTENSION);
            $img_ext = ['jpg', 'jpeg', 'png', 'gif'];
            if (!in_array($ext, $img_ext)) return false;
            $new_name = self::GenerateRandomString(10) . '.' . $ext;
            $new_path = $path . $new_name;
            if (!move_uploaded_file($file['tmp_name'], $new_path)) return false;
            return $new_name;

        } else if ($type == 'video') {
            $ext = pathinfo($file['name'], PATHINFO_EXTENSION);
            $video_ext = ['webm', 'mp4'];
            if (!in_array($ext, $video_ext)) return false;
            $new_name = self::GenerateRandomString(10) . '.' . $ext;
            $new_path = $path . $new_name;
            if (!move_uploaded_file($file['tmp_name'], $new_path)) return false;
            return $new_name;
            ...
            $ext = pathinfo($file['name'], PATHINFO_EXTENSION);
            $audio_ext = ['mp3', 'wav'];
            if (!in_array($ext, $audio_ext))  return false;
            $new_name = self::GenerateRandomString(10) . '.' . $ext;
            $new_path = $path . $new_name;
            if (!move_uploaded_file($file['tmp_name'], $new_path)) return false;
            return $new_name;
            ...
            $ext = pathinfo($file['name'], PATHINFO_EXTENSION);
            $text_ext = ['txt', 'doc', 'docx', 'pdf'];
            if (!in_array($ext, $text_ext)) return false;
            $new_name = self::GenerateRandomString(10) . '.' . $ext;
            $new_path = $path . $new_name;
            if (!move_uploaded_file($file['tmp_name'], $new_path)) return false;
            return $new_name;
            ...
            $new_name = self::GenerateRandomString(10). '.'. $ext;
            $new_path = $path. $new_name;
            return move_uploaded_file($file['tmp_name'], $new_path) ? $new_name : false;
        }
        
        return false;
    }
    static function ParseFile($file, $path, $new_ext) {
        $new_ext = str_replace('.', '', $new_ext);
        $newName = Utils::GenerateRandomString(10). '.'. $new_ext;
        $newPath = $path. $newName;
        return move_uploaded_file($file['tmp_name'], $newPath)? $newName : false;
    }
    static function GenerateRandomString(int $length = 10) {
    ...
    static function GetDateWithoutTime() {
        return date('Y-m-d');
    }
    static function GetDayElapse($date1, $date2) {
        $date1 = new DateTime($date1);
        $date2 = new DateTime($date2);
        $interval = $date1->diff($date2);
        return $interval->format('%a');
    }
    static function FormatToDate($year, $month, $day) {
    ...
    static function CountLines($filepath) {
    ...
    static function isEmailAddress($address) {
...
class CommonParser {
    ...
    protected function parseFunction(string $str): string {
        $openstack = 0;
        $inString = false;
        $backslash = false;
        $instrings = ['\'', '"', '`'];
        ...
        for ($i = 0; $i < strlen($str); $i++) {
            ...
            else if ($str[$i] == ')' && !$inString) $openstack--;
            else if (in_array($str[$i], $instrings) && !$backslash) $inString =!$inString;
            if($canBeClosed && $openstack == 0) { $last = $i; break; }

            if ($str[$i] == '\\' && $inString) $backslash = !$backslash;
            else if ($backslash) $backslash = false;
        }
...
class JavaScriptObject extends CommonParser{

    public function __construct(string $javascript) {
        $this->string = $javascript;
    }
    private function _setAsFunction(string $functionName) {
        $this->functionName = $functionName;
        $this->isFunction = true;
        return $this;
    }
    public function findFunction(string $functionName): JavaScriptObject|null {
        $str = strstr($this->string, $functionName);
        if ($str === false) return null;
        $newObj = new JavaScriptObject($this->parseFunction($str));
        return $newObj->_setAsFunction($functionName);
    }
    public function getParameters() { // TODO: implement getParameters
        throw new Exception('Not Implemented');
        /*if (!$this->isFunction) return null;
        $str = str_replace($this->functionName, '', $this->javascript);
        $str = substr($str, 1);
        $str = substr($str, 0, -1);*/

    }
    static public function getFuntion(string $file, string $functionName): string|null  {
        $obj = new JavaScriptObject($file);
        return $obj->findFunction($functionName)?->toString();
    }
}
class PHPObject extends CommonParser{
    public function __construct(string $php) {
        $this->string = $php;
    }
    public function findFunction(string $functionName): PHPObject|null {
        $str = strstr($this->string, $functionName);
        if ($str === false) return null;
        $obj = new PHPObject($this->parseFunction($str));
        return $obj->_setAsFunction($functionName);
    }
    private function _setAsFunction(string $functionName) {
        $this->functionName = $functionName;
        $this->isFunction = true;
