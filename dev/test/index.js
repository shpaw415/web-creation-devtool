...
import { ElementManager } from '../../WebApp.class.js';
...
new dev.Theme()
    ...
    self.add(self.create('div').html(theme.getElementId() ?? ''), self.create('input').set('placeholder', theme.getElementId() ?? ''));
}))
    .addAction(new dev.Action()
    .setName('TestAction')
    .setBuilder((action) => new ElementManager().call(self => {
    self.add(self.create('div').html('ActionBuilder'));
})))
    .setdata({
    background: 'black',
    opacity: '0.5',
